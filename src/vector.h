...
template <typename T>
T& Vector<T>::frontElement() const {
    return vector[0];
}

template <typename T>
T& Vector<T>::backElement() const {
...
template <typename T>
int Vector<T>::sizeOfVector() const {
    return size;
}

template <typename T>
int Vector<T>::capacityOfVector() const {
    return capacity;
}
...
template <typename T>
void Vector<T>::insertElement(int it, T value){
    if(it < 0 || it >= size){
...
template <typename T>
void Vector<T>::ereaseElement(int it){
    if(it < 0 || it >= size){
        throw std::out_of_range("Error: it out of range");
    } else {
